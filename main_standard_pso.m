%% AUV 三维路径规划 —— 标准 PSO（LPSO）对照实验
% 用途：与改进算法（DENPSO，main.m）做同环境、同参数的对照实验。
% 与 main.m 的唯一区别：
%   - 惯性权重取固定值 w=0.9（不做余弦自适应）
%   - 学习因子取固定值 c1=c2=1.5（不做正弦异步调整）
%   - 不启用距离进化因子与跳出机制，全区间按标准 PSO 更新
% 运行本脚本将生成 *11 系列结果数据（path11 / best_cost11 / energy11 /
% Violation11 / AUV_DATA11,22,33），供 compare_path / comp_cost / comp_energy
% / comp_vio / plot_AUV_MO 作为对照组读取。
...
%% 运行结束：保存对照实验结果数据（*11 系列）
% 说明：这些文件是算法运行的产物，已列入 .gitignore，不入版本控制。
%       供 compare_path / comp_cost / comp_energy / comp_vio / plot_AUV_MO 作为
%       对照组数据读取。保存的变量名与各读取脚本严格一致。

elapsed=toc;   %总运行耗时

save('best_cost11.mat','BestCost');   %收敛曲线（对照组） -> comp_cost
save('energy11.mat','energy_opt');    %能耗曲线（对照组） -> comp_energy
save('violation11.mat','Violation');  %约束违反次数（对照组） -> comp_vio
save('path11.mat','GlobalBest');      %最优路径（对照组） -> compare_path

%车辆运动信息（对照组） -> plot_AUV_MO
save('AUV_DATA11.mat','AUV_UNDER_POS1');
save('AUV_DATA22.mat','AUV_UNDER_POS2');
save('AUV_DATA33.mat','AUV_UNDER_POS3');
...
fprintf('耗时          : %.2f s\n', elapsed);
fprintf('对照数据已保存：best_cost11.mat / energy11.mat / violation11.mat / path11.mat / AUV_DATA11,22,33.mat\n');
